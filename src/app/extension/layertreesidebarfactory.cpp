	...
	SidebarFactory(parent)
{
	...
	setText(tr("Layers"));
	setCreationType(CreationTypeCanvas);
}

QWidget *LayerTreeSidebarFactory::createSidebarForCanvas(CanvasController *canvas, QWidget *parent)
{
	LayerTreeSidebar *sidebar;
	if (canvas)
	{
		sidebar = new LayerTreeSidebar(canvas->document()->layerModel(), parent);
	...
	{
		sidebar = new LayerTreeSidebar(0, parent);
	}
