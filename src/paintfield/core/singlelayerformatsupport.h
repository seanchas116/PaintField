...
public:
	...
	bool write(QIODevice *device, const QList<LayerConstRef> &layers, const QSize &size) override final;
	
	virtual bool readSingleLayer(QIODevice *device, Malachite::Surface *surface, QSize *size);
	virtual bool writeSingleLayer(QIODevice *device, const Malachite::Surface &surface, const QSize &size);
