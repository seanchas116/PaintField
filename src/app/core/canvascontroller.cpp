    ...
    QObject(parent)
{
	...
	_canvas = new Canvas(document);
	
	connect(parent->toolManager(), SIGNAL(currentToolFactoryChanged(ToolFactory*)), _canvas, SLOT(setToolFactory(ToolFactory*)));
	_canvas->setToolFactory(parent->toolManager()->currentToolFactory());
	
	addAction(new QAction(this), "saveDocument", this, SLOT(saveCanvas()));
	addAction(new QAction(this), "saveAsAction", this, SLOT(saveAsCanvas()));
	addAction(new QAction(this), "closeDocument", this, SLOT(closeCanvas()));
}
